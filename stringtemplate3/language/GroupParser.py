...
class Parser(antlr.LLkParser):
    ...
    def group(self, g):
        ...
        try:  ## for error handling
            ...
            while True:
                if (
                    (self.LA(1) == ID or self.LA(1) == AT)
                    and (
                        self.LA(2) == ID
                        or self.LA(2) == LPAREN
                        or self.LA(2) == DEFINED_TO_BE
                    )
                    and (self.LA(3) == ID or self.LA(3) == DOT or self.LA(3) == RPAREN)
                ):
                    self.template(g)
                elif (
                    (self.LA(1) == ID)
                    and (self.LA(2) == DEFINED_TO_BE)
                    and (self.LA(3) == LBRACK)
                ):
                    self.mapdef(g)
                else:
                    break
    ...
    def template(self, g):
        ...
        try:  ## for error handling
            if (self.LA(1) == ID or self.LA(1) == AT) and (
                self.LA(2) == ID or self.LA(2) == LPAREN
            ):
                la1 = self.LA(1)
                if False:
                    pass
    ...
    def arg(self, st):
        ...
        try:  ## for error handling
            ...
            if (self.LA(1) == ASSIGN) and (self.LA(2) == STRING):
                ...
                s = self.LT(1)
                self.match(STRING)
                defaultValue = stringtemplate3.StringTemplate(template="$_val_$")
    ...
    def keyValue(self):
        ...
        try:  ## for error handling
            ...
            if False:
                ...
                self.match(BIGSTRING)
                value = stringtemplate3.StringTemplate(
                    group=self.group_, template=s2.getText()
                )
            elif la1 and la1 in [ID]:
                k = self.LT(1)
                self.match(ID)

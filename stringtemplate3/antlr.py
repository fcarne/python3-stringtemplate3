...
class NoViableAltException(RecognitionException):
    ...
    def __str__(self):
        if self.token:
            ...
            return 'unexpected symbol at line {} (column {}): "{}"'.format(
                line, col, text
            )
...
class InputBuffer:
    ...
    def __str__(self):
        return "({},{},{},{})".format(
            self.nMarkers, self.markerOffset, self.numToConsume, self.queue
        )
...
class CharBuffer(InputBuffer):
    ...
    def __str__(self):
        base = super().__str__()
        return f"CharBuffer{{{base},{str(input)}"
...
class CharScanner(TokenStream):
    ...
    def traceIn(self, rname):
        self.traceDepth += 1
        self.traceIndent()
        print(f"> lexer {rname} c== {self.LA(1)}")
...
def illegalarg_ex(func):
    raise ValueError(
        "%s is only valid if parser is built for debugging" % (func.__name__)
    )
...
class TokenBuffer:
    ...
    def __str__(self):
        return "({},{},{},{},{})".format(
            self.input, self.nMarkers, self.markerOffset, self.numToConsume, self.queue
        )
